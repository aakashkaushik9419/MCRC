<?php
 ...
 if(isset($_POST['submit'])){
   ...
   $select_users = mysqli_query($conn, "SELECT * FROM `Query` WHERE Email = '$Email' AND Subject = '$Subject'") or die('query failed');

   if(mysqli_num_rows($select_users) > 0){
      echo 'Sorry, ',$Name; echo'! You have already contacted regarding ',$Subject;
      }else{
         mysqli_query($conn, "INSERT INTO `Query`(Name, Email, Message, Subject) VALUES('$Name', '$Email', '$Message', '$Subject')") or die('query failed');
